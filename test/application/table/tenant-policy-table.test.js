...
let TenantPolicyTable;
beforeAll(() => {
  TenantPolicyTable = require(globalThis.applicationPath('/application/table/tenant-policy-table'));
});

describe('TenantPolicyTable', () => {
  let table;
  let mockAdapter;
  let mockSelectQuery;
  ...
  describe('fetchById', () => {
    it('should return null when no rows found', async () => {
      ...
      expect(mockSelectQuery.where).toHaveBeenCalledWith('tp.policy_id = ?', 'p-1');
  ...
  describe('fetchByTenantId', () => {
    it('should query by tenant_id', async () => {
      ...
      expect(mockSelectQuery.where).toHaveBeenCalledWith('tp.tenant_id = ?', 't-1');
      expect(mockSelectQuery.limit).toHaveBeenCalledWith(1);
      expect(result).toBeNull();
    });
  });

  describe('fetchDtoByTenantId', () => {
    it('should return null when no rows', async () => {
      const result = await table.fetchDtoByTenantId('t-1');
      expect(mockSelectQuery.where).toHaveBeenCalledWith('tp.tenant_id = ?', 't-1');
      expect(result).toBeNull();
    });

    it('should return DTO when row found', async () => {
      mockSelectQuery.execute.mockResolvedValue({ rows: [{ policy_id: 'p-1', tenant_id: 't-1' }] });
      const result = await table.fetchDtoByTenantId('t-1');
  ...
  describe('fetchByTenantId - found', () => {
    it('should return entity when row found', async () => {
      mockSelectQuery.execute.mockResolvedValue({ rows: [{ policy_id: 'p-1', tenant_id: 't-1' }] });
      const result = await table.fetchByTenantId('t-1');
      expect(result).not.toBeNull();
  ...
  describe('getSelectQuery', () => {
    it('should return a Select instance', async () => {
      const realTable = new TenantPolicyTable({ adapter: mockAdapter });
  ...
  describe('entityFactory', () => {
    it('should create entity from row data', () => {
      if (table.entityFactory) {
        const entity = table.entityFactory({ policy_id: 'p-1', tenant_id: 't-1' });
