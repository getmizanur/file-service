...
let EmailVerificationTokenTable;
beforeAll(() => {
  EmailVerificationTokenTable = require(globalThis.applicationPath('/application/table/email-verification-token-table'));
});

describe('EmailVerificationTokenTable', () => {
  let table;
  let mockAdapter;
  let mockSelectQuery;
  ...
  beforeEach(() => {
    ...
    table = new EmailVerificationTokenTable({ adapter: mockAdapter });
    table.getSelectQuery = jest.fn().mockResolvedValue(mockSelectQuery);
  });

  describe('constructor', () => {
    it('should set the table name', () => {
      expect(table.table).toBe('email_verification_token');
    });
  ...
  describe('fetchByUserId', () => {
    it('should query by user_id and return array', async () => {
  ...
  describe('fetchByUserWithDetails', () => {
    it('should build join query for user details', async () => {
      const result = await table.fetchByUserWithDetails('u-1');
      expect(mockSelectQuery.from).toHaveBeenCalledWith({ t: 'email_verification_token' }, []);
