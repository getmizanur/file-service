...
let UserGroupTable;
beforeAll(() => {
  UserGroupTable = require(globalThis.applicationPath('/application/table/user-group-table'));
});
...
describe('UserGroupTable', () => {
  ...
  describe('fetchById', () => {
    it('should return null when no rows found', async () => {
      const result = await table.fetchById('g-1');
      expect(mockSelectQuery.where).toHaveBeenCalledWith('group_id = ?', 'g-1');
      expect(result).toBeNull();
    });

    it('should return entity when found', async () => {
      mockSelectQuery.execute.mockResolvedValue({ rows: [{ group_id: 'g-1' }] });
      const result = await table.fetchById('g-1');
  ...
  describe('fetchByName', () => {
    it('should query by tenant and name and return null when not found', async () => {
      const result = await table.fetchByName('t-1', 'admins');
      expect(mockSelectQuery.where).toHaveBeenCalledWith('tenant_id = ?', 't-1');
      expect(mockSelectQuery.where).toHaveBeenCalledWith('name = ?', 'admins');
      expect(result).toBeNull();
    });

    it('should return entity when found', async () => {
      mockSelectQuery.execute.mockResolvedValue({ rows: [{ group_id: 'g-1', name: 'admins' }] });
      const result = await table.fetchByName('t-1', 'admins');
      expect(result).not.toBeNull();
    });
  });
  ...
  describe('fetchByTenantWithDetails', () => {
    it('should build join query with grouping', async () => {
      ...
      expect(mockSelectQuery.from).toHaveBeenCalledWith({ g: 'user_group' }, []);
      expect(mockSelectQuery.joinLeft).toHaveBeenCalled();
      expect(mockSelectQuery.groupBy).toHaveBeenCalledWith('g.group_id');
      expect(mockSelectQuery.where).toHaveBeenCalledWith('g.tenant_id = ?', 't-1');
      expect(Array.isArray(result)).toBe(true);
    });
  });
  ...
  describe('insert', () => {
    it('should return entity on success', async () => {
      mockAdapter.query = jest.fn().mockResolvedValue({ rows: [{ group_id: 'g-1' }], rowCount: 1 });
      const result = await table.insert('t-1', 'new-group');
      expect(result).not.toBeNull();
  ...
  describe('update', () => {
    it('should execute update query', async () => {
      ...
      const result = await table.update('g-1', 'renamed-group');
      expect(mockAdapter.query).toHaveBeenCalledWith(
  ...
  describe('deleteById', () => {
    it('should execute delete query', async () => {
      mockAdapter.query = jest.fn().mockResolvedValue({ rows: [], rowCount: 1 });
      const result = await table.deleteById('g-1');
      expect(mockAdapter.query).toHaveBeenCalledWith(
