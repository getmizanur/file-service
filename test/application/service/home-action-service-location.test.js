...
const HomeActionService = require(globalThis.applicationPath('/application/service/action/home-action-service'));
const BreadcrumbHelper = require(globalThis.applicationPath('/application/helper/breadcrumb-helper'));

describe('HomeActionService - Location Annotations', () => {
  let service;
  ...
  describe('BreadcrumbHelper.buildBreadcrumbs()', () => {
    it('builds breadcrumbs for nested folder', () => {
      const crumbs = BreadcrumbHelper.buildBreadcrumbs('sub', 'root', folders);
    ...
    it('builds single breadcrumb for root folder', () => {
      const crumbs = BreadcrumbHelper.buildBreadcrumbs('root', 'root', folders);
      expect(crumbs).toEqual([
        { name: 'My Drive', folder_id: 'root' }
      ]);
    });
    ...
    it('returns default when no currentFolderId', () => {
      const crumbs = BreadcrumbHelper.buildBreadcrumbs(null, 'root', folders);
      expect(crumbs).toEqual([{ name: 'My Drive', folder_id: 'root' }]);
    });

    it('returns default when no rootFolderId', () => {
      const crumbs = BreadcrumbHelper.buildBreadcrumbs('docs', null, folders);
