...
class RenderFolderTreeHelper extends AbstractHelper {
  ...
  _renderRecursive(items, level, activeId, viewMode, layoutMode, expandedIds) {
    ...
    items.forEach(item => {
      const result = this._renderItem(item, level, activeId, viewMode, layoutMode, expandedIds);
      html += result.html;
      if (result.isActive) {
  ...
  _renderItem(item, level, activeId, viewMode, layoutMode, expandedIds) {
    ...
    return { html, isActive: shouldExpand };
  }
}
