...
class StarredActionService extends AbstractActionService {
  ...
  async list({ userEmail, identity, layoutQuery = null, sortQuery = null }) {
    ...
    const _t = (label, fn) => profiler?.isEnabled() ? profiler.time(label, fn, { parent: 'StarredActionService' }) : fn();
    ...
    const subFolders = await qcs.cacheThrough(
    ...
    // For starred view, resolve full file details from starred file IDs
    const filesList = await _t('fetchStarredFileDetails', () => this._fetchStarredFileDetails(fileMetadataService, starredFileIds));

    let folderTree = [];
    try {
      folderTree = folderService.buildFolderTree(folders);
    } catch (e) {
      console.error('[StarredActionService] Error building folder tree:', e);
    }

    const starredFolderIds = subFolders.map(f => f.folder_id);
    ...
    const plainFiles = filesList.map(toPlain);
    const plainSubFolders = subFolders.map(toPlain);
  ...
  async _resolveRootFolder(qcs, folderService, userEmail, emailHash) {
    ...
    try {
      ...
      console.error('[StarredActionService] Error resolving root folder:', e);
    }
  ...
  async _fetchStarredFileIds(qcs, sm, userEmail, emailHash, userReg) {
    try {
      ...
      console.error('[StarredActionService] Error fetching starred files:', e);
      return [];
    }
  }
  ...
  async _fetchStarredFileDetails(fileMetadataService, starredFileIds) {
    try {
      ...
      console.error('[StarredActionService] Error fetching starred file details:', e);
      return [];
    }
  }
  ...
  async _populateSharedFlags(mergedItems, sm, tenantId, qcs) {
    try {
      ...
      console.error('[StarredActionService] Error populating is_shared:', e);
    }
  }
  ...
  async _populateDerivativeFlags(mergedItems, sm, qcs = null, tenantId = null) {
    try {
      ...
      console.error('[StarredActionService] Error populating has_thumbnail:', e);
    }
  }
...
module.exports = StarredActionService;
