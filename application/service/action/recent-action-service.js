...
class RecentActionService extends AbstractActionService {
  ...
  async list({ userEmail, identity, layoutQuery = null, sortQuery = null }) {
    ...
    const _t = (label, fn) => profiler?.isEnabled() ? profiler.time(label, fn, { parent: 'RecentActionService' }) : fn();
    ...
    try {
      ...
      console.error('[RecentActionService] Error building folder tree:', e);
    }
  ...
  async _resolveRootFolder(qcs, folderService, userEmail, emailHash) {
    ...
    try {
      ...
      console.error('[RecentActionService] Error resolving root folder:', e);
    }
  ...
  async _fetchStarredFileIds(qcs, sm, userEmail, emailHash, userReg) {
    try {
      ...
      console.error('[RecentActionService] Error fetching starred files:', e);
      return [];
    }
  }
  ...
  async _fetchStarredFolderIds(qcs, folderStarService, tenantId, identity, userReg) {
    try {
      ...
      console.error('[RecentActionService] Error fetching starred folder IDs:', e);
      return [];
    }
  }
  ...
  async _populateSharedFlags(mergedItems, sm, tenantId, qcs) {
    try {
      ...
      console.error('[RecentActionService] Error populating is_shared:', e);
    }
  }
  ...
  async _populateDerivativeFlags(mergedItems, sm, qcs = null, tenantId = null) {
    try {
      ...
      console.error('[RecentActionService] Error populating has_thumbnail:', e);
    }
  }
...
module.exports = RecentActionService;
