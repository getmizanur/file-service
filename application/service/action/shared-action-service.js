// application/service/action/shared-action-service.js
/* eslint-disable no-undef */
const AbstractActionService = require(globalThis.applicationPath('/application/service/abstract-action-service'));
const crypto = require('node:crypto');

class SharedActionService extends AbstractActionService {

  async list({ userEmail, identity, folderId, layoutQuery = null, sortQuery = null }) {
    ...
    const _t = (label, fn) => profiler?.isEnabled() ? profiler.time(label, fn, { parent: 'SharedActionService' }) : fn();
    ...
    try {
      ...
      console.error('[SharedActionService] Error fetching subfolders:', e);
    }
    ...
    try {
      ...
      console.error('[SharedActionService] Error building folder tree:', e);
    }
    ...
    const plainSubFolders = []; // shared-with-me view does not show subfolders in listing
  ...
  async _resolveRootFolder(qcs, folderService, userEmail, emailHash) {
    ...
    try {
      ...
      console.error('[SharedActionService] Error resolving root folder:', e);
    }
  ...
  _resolveCurrentFolder(folderId, rootFolder, folders, userEmail) {
    ...
    if (currentFolderId && currentFolderId !== rootFolderId) {
      ...
      if (!tenantFolderIds.has(currentFolderId)) {
        console.warn(`[SharedActionService] Unauthorized folder access attempt: ${currentFolderId} by ${userEmail}`);
  ...
  async _fetchStarredFileIds(qcs, sm, userEmail, emailHash, userReg) {
    try {
      ...
      console.error('[SharedActionService] Error fetching starred files:', e);
      return [];
    }
  }
  ...
  async _fetchStarredFolderIds(qcs, folderStarService, tenantId, identity, userReg) {
    try {
      ...
      console.error('[SharedActionService] Error fetching starred folder IDs:', e);
      return [];
    }
  }
  ...
  async _populateSharedFlags(mergedItems, sm, tenantId, qcs) {
    try {
      ...
      console.error('[SharedActionService] Error populating is_shared:', e);
    }
  }
  ...
  async _populateDerivativeFlags(mergedItems, sm, qcs = null, tenantId = null) {
    try {
      ...
      console.error('[SharedActionService] Error populating has_thumbnail:', e);
    }
  }
...
module.exports = SharedActionService;
