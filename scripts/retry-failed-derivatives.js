...
// Validate
// ------------------------------------------------------------------
if (!modeAll && !tenantId && !fileId) {
  console.error('Error: one of --all, --tenant <id>, or --file <id> is required.\n');
  console.error('Usage:');
...
async function main() {
  ...
  try {
    ...
    // Fetch one row per file that has at least one failed derivative.
    // Join file_metadata to get everything generateDerivatives() needs.
